...
class Agent{
  ...
  run(selected_search){
    ...
    rect(left+10, top+10, sizec-20, sizec-20)
    
    this.setSearch(selected_search);

    // The agent performs the search considering its current position as the initial state
    if (!this.found && !this.done) this.search(this.grid);
   ...
   *
   * @param {Cell[][]} grid - grid where the search will be performed.
   */
  search(grid){
