...
class World{
  ...
  constructor(cell_size, padding, rnum, snum, lnum){
    ...
    this.food_textr = loadImage('assets/steak_pixel.png');
    // this.food_rotten = loadImage('assets/rotten_pixel.png');
  }
  ...
  run(){
    ...
    for (let col=0; col<this.columns; col++) {
      for (let row=0; row<this.rows; row++) {
        ...
        image(this.cells[col][row].typetexture, left, top);  // Placing texture
        rect(left, top, sizec, sizec);  // Representing cells as squares
        
        // Food will appear as a chicken piece
        if (this.cells[col][row].has_food){  
          image(this.food_textr, left+10, top+9);
          noFill();
        }
        // if (this.cells[col][row].is_rotten){  
        //   image(this.food_rotten, left+10, top+9);
        //   noFill();
        // }
      }
    }
